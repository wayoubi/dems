...
import ca.concordia.ginacody.comp6231.services.EventManagementServiceCORBAImpl;
...
public class CorbaServer implements Runnable {
    ...
    public void run() {
        try {
            ...
            NameComponent path[] = ncRef.to_name(Configuration.SERVER_LOCATION);
            ncRef.rebind(path, href);

            LOGGER.info(String.format("DEMS CORBA Server %s ready and waiting ...", Configuration.SERVER_LOCATION));
        ...
        LOGGER.info(String.format("DEMS CORBA Server %s Exiting ...", Configuration.SERVER_LOCATION));
    }
